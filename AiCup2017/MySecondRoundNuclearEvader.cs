...
{
    public class MySecondRoundNuclearEvader : MyINuclearEvader
    {
        ...
        public bool CheckNuclearAlarm()
        {
            if (_str.GroupingEnded)
            {
                if (_str.World.GetOpponentPlayer().RemainingNuclearStrikeCooldownTicks > 512 && _str.EnemyNuclearReady)
                {
                    ...
                    _str.NuclearGameTasks.Enqueue(_str.Act.SelectByFrameTask(new MyPoint(_nuclearPoint.X - 25, _nuclearPoint.Y - 25), new MyPoint(_nuclearPoint.X + 25, _nuclearPoint.Y + 25)));
                ...
                if (!_str.EnemyNuclearReady)
                {
                    ...
                    if (_str.World.TickIndex == _nuclearAlarmTick + 31)
                    {
                        ...
                        _str.NuclearGameTasks.Enqueue(_str.Act.SelectByGroup((int)_str.CurrentGroup));
                    }
